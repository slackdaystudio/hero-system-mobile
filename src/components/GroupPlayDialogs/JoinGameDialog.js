...
export default class JoinGameDialog extends Component {
    ...
    _save() {
        if (this.props.username === null || this.props.username.length < 2) {
            ...
            return;
        } else if (this.props.ip === null || !/^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$/.test(this.props.ip)) {
            this.setState({errorMessage: 'Please supply a valid IPv4 address'});
                ...
                <View style={styles.modal}>
                    ...
                    <View style={styles.modalContent}>
                        ...
                        <Form style={{paddingBottom: verticalScale(15)}}>
                            ...
                            <Item inlineLabel style={{marginLeft: 0}}>
                                <Label style={styles.grey}>IP Address:</Label>
                                <Input
                                    style={styles.grey}
                                    maxLength={15}
                                    value={this.props.ip}
                                    onChangeText={(value) => this.props.updateIp(value)}
                                />
                            </Item>
                        </Form>
                        <View style={{flex: 1, flexDirection: 'row', justifyContent: 'space-around', paddingTop: verticalScale(30)}}>
                            <Button style={styles.button} onPress={() => this._save()}>
                                <Text uppercase={false} style={styles.buttonText}>Join Game</Text>
                            </Button>
