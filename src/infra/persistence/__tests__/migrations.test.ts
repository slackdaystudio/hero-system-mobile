...
describe('migrations (real SQLite)', () => {
    it('applies the base schema and records the version', () => {
        ...
        expect(db.execute('SELECT version FROM schema_migrations ORDER BY version').rows).toEqual([{version: 1}, {version: 2}, {version: 3}, {version: 4}, {version: 5}]);
    ...
    it('is idempotent — re-running applies nothing', () => {
        ...
        expect(db.execute('SELECT COUNT(*) AS n FROM schema_migrations').rows[0].n).toBe(5);
    });
