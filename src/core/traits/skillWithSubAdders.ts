...
/** Sub-adder skill cost (Forgery, Gambling, Survival), ported from legacy `SkillWithSubAdders.js`. */
export default class SkillWithSubAdders extends TraitDecorator {
    cost(): number {
        let cost = 3;
        const trait = this.characterTrait.trait;

        if (hasOwn(trait, 'adder')) {
            cost = this.totalAdders(trait.adder);
        }
